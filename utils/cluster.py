...
def auto_select_k(silhouette_scores, k_range, display_top_n=5):
...
def prepare_train_data(df, optimal_k, output_cluster_file, output_dict_file):
    ...
    with open(output_dict_file, 'w', encoding='utf-8') as dict_label_file:
...
# Visualization
def visualize_color_clusters(df, optimal_k):
...
def cluster_kmeans(input_file, output_cluster_file, output_dict_file):
    ...
    optimal_k = auto_select_k(silhouette_scores, list(k_range), display_top_n=5)
    print(f"Suggest K: {optimal_k}")

    final_kmeans = KMeans(n_clusters=optimal_k, random_state=42, n_init=10)
    df['cluster'] = final_kmeans.fit_predict(X_scaled)

    analyze_clusters(df, optimal_k)

    df_labeled, cluster_names = prepare_train_data(df, optimal_k, output_cluster_file, output_dict_file)
    print("\n")
    ...
    visualize_color_clusters(df_labeled, optimal_k)
